...
% Iteration 6
\subsection*{Iteration 6: Pop bewegt Head in falsche Richtung}
	extbf{Änderung:} \texttt{pop()} setzt \texttt{head = (head - 1 + buffer.length) \% buffer.length;} anstelle von
	exttt{head = (head + 1) \% buffer.length}.
\textbf{Erwartete fehlschlagende Tests:} \texttt{testPop1}, \texttt{testPop2}, \texttt{testPop3}
\textbf{Warum:} Pop entfernt das falsche Element 
und die Reihenfolge der ältesten Elemente ändert sich.
